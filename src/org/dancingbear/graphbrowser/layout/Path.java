...
public class Path {
    ...
    public Path() {
        ...
        grownSegments = new ArrayList<Segment>();
        points = new PointList();
        visibleVertices = new HashSet<Vertex>();
        stack = new SegmentStack();
        visibleObstacles = new HashSet<Obstacle>();
        excludedObstacles = new ArrayList<Obstacle>();
    }
    ...
    private void addConnectingSegment(Segment segment, Obstacle o1,
        ...
        if (threshold != 0
                && (segment.end.getDistance(end)
                        ...
                        .getDistance(end)
                        + segment.start.getDistance(start) > threshold))
            return;
    ...
    private void addSegment(Segment segment, Obstacle exclude1,
            Obstacle exclude2, List<Obstacle> allObstacles) {
        if (threshold != 0
                && (segment.end.getDistance(end)
                        + segment.end.getDistance(start) > threshold || segment.start
                        .getDistance(end)
 ...
 */
    ...
    Path getSubPath(Segment currentSegment) {
        ...
        newPath.grownSegments = new ArrayList<Segment>(grownSegments.subList(
                grownSegments.indexOf(currentSegment), grownSegments.size()));

        // fix old path
        grownSegments = new ArrayList<Segment>(grownSegments.subList(0,
