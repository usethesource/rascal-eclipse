...
class RankAssignmentSolver extends SpanningTreeVisitor {
    ...
    int depthFirstCutValue(Edge edge, int count) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (e.isTree() && e != edge) {
                ...
                cutvalue += (e.getCut() - e.getWeight()) * multiplier;
            } else {
        ...
        for (int i = 0; i < graph.getNodes().size(); i++) {
            ...
            if (subtreeContains(branch, n)) {
                ...
                for (int j = 0; j < edges.size(); j++) {
                    ...
                    if (!subtreeContains(branch, e.opposite(n)) && !e.isTree()
                            && e.getSlack() < minSlack) {
                        result = e;
                        minSlack = e.getSlack();
                    }
    ...
    Edge leave() {
        ...
        for (int i = 0; i < spanningTree.size(); i++) {
            ...
            if (e.getCut() < minCut) {
                result = e;
                minCut = result.getCut();
                weight = result.getWeight();
            } else if (e.getCut() == minCut && e.getWeight() > weight) {
                result = e;
                weight = result.getWeight();
            }
        }
