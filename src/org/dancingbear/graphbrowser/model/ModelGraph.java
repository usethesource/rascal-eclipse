...
class ModelGraph extends PropertyContainer implements IModelGraph {
	...
	private ConcurrentMap<Integer, IModelEdge> edges;
	private ConcurrentMap<String, IModelNode> nodes;
	private ConcurrentMap<Integer, IModelSubgraph> subgraphs;
	...
	public ModelGraph(Integer graphId, String name,
		...
		edges = new ConcurrentHashMap<Integer, IModelEdge>();
		nodes = new ConcurrentHashMap<String, IModelNode>();
		subgraphs = new ConcurrentHashMap<Integer, IModelSubgraph>();
	}
	...
	public  void clearGraph() {
		...
		for (IModelNode node: nodes.values()) {
			removeNode(node);	
		}	
	...
	public  boolean containsEdge(IModelEdge edge) {
		return edge != null && edges.containsKey(edge.getId()) && edges.get(edge.getId()).equals(edge);
	}
