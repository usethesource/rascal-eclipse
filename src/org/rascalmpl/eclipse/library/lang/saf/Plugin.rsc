...
private node outlineSAF(start[Fighter] fighter) {
  ...
  attrs = ["attr"(label="<s.name>: <s.strength>",origin=s.origin) | /Spec s := fighter, s is attribute ];
  behvs = ["behv"(
         "move"(label="move: <s.move>",origin=((s.move).origin)),
         "fight"(label="fight: <s.fight>",origin=((s.fight).origin))
      ,label="<s.cond>",origin=s.origin) | /Spec s := fighter, s is behavior ];
  name = ( "" | "<f.name>" | /Fighter f := fighter );
  return "fighter"(
            "attrs"(attrs,label="Strengths"),
            "behvs"(behvs,label="Behaviors")
          ,label="<name>",origin=fighter.origin);
}
