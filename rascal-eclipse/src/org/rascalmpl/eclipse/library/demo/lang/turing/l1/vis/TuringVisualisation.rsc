...
public void visInterpreter(Program prog){
	int fps = 12;
	bool hadChange = true;
	TuringState state = initialState(prog,[false | _ <- [1..tapeSize+1]]);
	bool running = false;
	tape = hcat([box(computeFigure(Figure() { return (state.pos == i) ? ellipse(fillColor("red"),shrink(0.5)) : space(); }),
		 fillColor(Color () {
		 	return state.tape[i] ? color("green") : color("white"); }), onMouseDown(bool (int z, map[KeyModifier,bool] _) { 
		 	  if (z == 1) {
		 	    state.tape[i] = !state.tape[i];
		 	  } else { state.pos=i; } return false; 
		 	})
	...
	progView = hcat([vcat([
		box(text("<i+1>\t<compile(state.prog[i])>",left()), fillColor(Color () { return (i + 1 == state.line) ? color("green") : color("white"); }), 
				onMouseDown(bool (int _, map[KeyModifier,bool] _) { state.line = i + 1; return false; }))
